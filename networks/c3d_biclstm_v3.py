  ...
  with tf.device('/gpu:0'):
    with tf.variable_scope('Conv3D_ConvLSTM', reuse=reuse):
      ...
      classes = tl.layers.DropconnectDenseLayer(flatten_1, 
                                        ...
                                        act=tf.identity,
                                        name='Classes')
      classes = tl.layers.ReshapeLayer(layer=classes,
                                        shape=[-1, num_steps, num_classes, 1],
                                        name='Classes_reshape_1')
      classes = tl.layers.PoolLayer(classes,
                                        ksize=[1, num_steps, 1, 1],
                                        strides=[1, num_steps, 1, 1],
                                        padding='SAME',
                                        pool = tf.nn.avg_pool,
                                        name='Classes_Pool')
      classes = tl.layers.ReshapeLayer(layer=classes,
                                        shape=[-1, num_classes],
                                        name='Classes_reshape_2')
    return classes
