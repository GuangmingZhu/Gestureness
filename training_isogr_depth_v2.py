...
training_datalist = './dataset_splits/IsoGD/train_depth_list.txt'
...
l2_cost = tf.contrib.layers.l2_regularizer(weight_decay)(networks.all_params[0]) + \
          ...
          tf.contrib.layers.l2_regularizer(weight_decay)(networks.all_params[34]) + \
          tf.contrib.layers.l2_regularizer(weight_decay)(networks.all_params[40]) + \
          tf.contrib.layers.l2_regularizer(weight_decay)(networks.all_params[46])
...
lr2 = tf.train.exponential_decay(learning_rate*10,
                                ...
                                decay_rate,
                                staircase=True)
var_list1 = networks.all_params[:46] 
var_list2 = networks.all_params[46:]
...
if start_step>0:
  load_params = tl.files.load_npz(name='%s/%s_rnn_model_epoch_%d.npz'%(model_prefix, dataset_name, start_epoch))
  ...
  pretrained_model = './trained_models/v2/isogr_pretrained_model.npz'
  load_params = tl.files.load_npz(name=pretrained_model)
  tl.files.assign_params(sess, load_params[0:46], networks)
...
for epoch in range(start_epoch, n_epoch):
  ...
  tl.files.save_npz(networks.all_params, 
                    name='%s/%s_rnn_model_epoch_%d.npz'%(model_prefix, dataset_name, epoch+1), 
                    sess=sess)
  print("Model saved in file: %s_rnn_model_epoch_%d.npz" %(dataset_name, epoch+1))
  log.writelines(['Model saved in file: %s_rnn_model_epoch_%d.npz'%(dataset_name, epoch+1), 
                  '\n'])
  log.flush()
